...
private:
    ...
    // 2. 常量声明
    const int INITIAL_CWND = 1; // 初始拥塞窗口大小
    const int INITIAL_SSTHRESH = 32;// 初始慢启动阈值
    const int INITIAL_TIMEOUT = 1000;  // 新增：初始超时时间设置
    ...
    bool simulate_timeout = false;  // 新增：超时模拟控制标志
    int current_timeout;  // 新增：当前超时时间

    int delay_ms;//延迟
    int cwnd;  // 拥塞窗口
    int ssthresh;// 慢启动阈值
    int dup_acks;// 重复ACK计数
    int last_ack; // 上一个收到的ACK    
    CongestionState current_state;  // 使用上面定义的枚举类型
public:
    UDPCLIENT() : Client_Socket(INVALID_SOCKET),
        ...
        current_timeout(INITIAL_TIMEOUT),  // 新增
        simulate_timeout(false)            // 新增
    {
        cout << "[初始化] 初始ssthresh = " << ssthresh << endl;
        cout << "[初始化] 初始cwnd = " << cwnd << endl;
        cout << "[初始化] 初始timeout = " << current_timeout << "ms" << endl;
    }
    // 添加设置模拟参数的方法
    ...
    void Set_SimulateTimeout(bool enable) {
        simulate_timeout = enable;
        cout << "[系统] " << (enable ? "启用" : "禁用") << "超时模拟" << endl;
    }

    void Set_Timeout(int timeout_ms) {
        current_timeout = timeout_ms;
        cout << "[系统] 设置超时时间为: " << timeout_ms << "ms" << endl;
    }
    bool INIT() {
        // 初始化
        WSADATA wsaData;
    ...
    void Print_CongestionState() {
        ...
        SetConsoleTextAttribute(hConsole, 13); // 粉色
        cout << "[拥塞控制] 当前状态: " << state_str
            << " | cwnd: " << cwnd
            << " | ssthresh: " << ssthresh
            << " | 重复ACK数: " << dup_acks
            << " | Base: " << Base_Seq
    ...
    // 更新拥塞控制窗口
    void Update_CongestionWindow(bool is_timeout) {
        if (is_timeout) {
            // 超时事件处理
            ...
            cout << "[超时] 超时事件: ssthresh = " << ssthresh << ", cwnd = " << cwnd << endl;
            SetConsoleTextAttribute(hConsole, 7);
        }
        else {
            // 根据当前状态更新窗口
            ...
            case SLOW_START://慢启动
                ...
                if (cwnd >= ssthresh) {
                    ...
                    SetConsoleTextAttribute(hConsole, 13);
                    cout << "[拥塞避免] 达到阈值，从慢启动切换到拥塞避免阶段" << endl;
                    SetConsoleTextAttribute(hConsole, 7);
                }
                else {
                    cwnd *= 2;//指数增长
                    SetConsoleTextAttribute(hConsole, 13);
                    cout << "[慢启动] 慢启动阶段: cwnd加倍为" << cwnd << endl;
                    SetConsoleTextAttribute(hConsole, 7);
                }
                break;

            case CONGESTION_AVOIDANCE://拥塞避免
                cwnd += 1.0 ;  // 每个RTT线性增加1个MSS
                SetConsoleTextAttribute(hConsole, 13);
                cout << "[拥塞避免] 拥塞避免阶段: cwnd增加1 现在为：" << cwnd << endl;
                SetConsoleTextAttribute(hConsole, 7);
                break;

            case FAST_RECOVERY://快速恢复
                current_state = CONGESTION_AVOIDANCE;
                cwnd = ssthresh;
                SetConsoleTextAttribute(hConsole, 13);
                cout << "[拥塞控制] 快速恢复结束: cwnd设置为ssthresh=" << ssthresh << endl;
                SetConsoleTextAttribute(hConsole, 7);
                break;
            }
        }
        Print_CongestionState();
    }

    // 处理重复ACK
    void Handle_DuplicateAck(int ack_num) {
        if (ack_num == last_ack) {
            dup_acks++;
            if (dup_acks == 3) {
                // 进入快速重传和快速恢复阶段
                SetConsoleTextAttribute(hConsole, 13);
                cout << "[拥塞控制] 收到三个重复ACK，进入快速重传和快速恢复阶段" << endl;
                SetConsoleTextAttribute(hConsole, 7);
                // 快速重传
                ssthresh = max(cwnd / 2, 2);
                cwnd = ssthresh + 3;
                ...
                SetConsoleTextAttribute(hConsole, 13);
                cout << "[快速重传] 快速重传: ssthresh=" << ssthresh << ", cwnd=" << cwnd << endl;
                SetConsoleTextAttribute(hConsole, 7);
                Resend = true;
                Print_CongestionState();
            }
            else if (current_state == FAST_RECOVERY) {
                // 在快速恢复阶段，每收到一个重复ACK就增加cwnd
                cwnd += 1;
                SetConsoleTextAttribute(hConsole, 13);
                cout << "[快速恢复] 快速恢复中收到重复ACK: cwnd增加1，现在为" << cwnd << endl;
                SetConsoleTextAttribute(hConsole, 7);
            }
        }
        else {
    ...
    void Fast_Ack() {
        chrono::steady_clock::time_point last_ack_time = chrono::steady_clock::now();//时间记录机制
        ...
        while (true) {
            ...
            if (recvfrom(Client_Socket, (char*)&ack_msg, sizeof(ack_msg), 0, (SOCKADDR*)&Router_Address, &addr_len)) {
                if (ack_msg.Is_ACK() && ack_msg.Check_IsValid()) {
                    last_ack_time = chrono::steady_clock::now();//在每次接收到ACK时，会更新最后接收ACK的时间
                    ...
                    if (ack_msg.ack >= Base_Seq + Head_Seq) {
                        // 收到新的ACK
                        Base_Seq = ack_msg.ack - Head_Seq + 1;
                        if (timeout_occurred) {
                            timeout_occurred = false;                           
                        }
                        Update_CongestionWindow(false); // 正常ACK，更新窗口
                    }
                    else {
                        // 处理重复ACK
                        Handle_DuplicateAck(ack_msg.ack);
                    }
            ...
            if (chrono::duration_cast<chrono::milliseconds>(current_time - last_ack_time).count() > current_timeout) {
                if (simulate_timeout) {  // 启用超时模拟时执行
                    timeout_occurred = true;
                    SetConsoleTextAttribute(hConsole, 12); // 红色
                    cout << "[超时] 检测到超时事件!" << endl;
                    SetConsoleTextAttribute(hConsole, 7);

                    Update_CongestionWindow(true);
                    Resend = true;// 标记需要重传
                    dup_acks = 0;
                }
                last_ack_time = current_time; // 重置计时器
            }
        }
    }
    ...
    void Print_WindowStatus() {
        ...
        static int last_cwnd = -1;

        // 如果 Base_Seq 或 Next_Seq 发生变化，更新窗口状态
        if (last_base != Base_Seq || last_next != Next_Seq || last_cwnd != cwnd) {
    ...
    bool Send_Message(string file_path) {
        ...
        while (!Completed) {
            ...
            // 正常发送处理
            if (Next_Seq < Base_Seq + cwnd && Next_Seq <= Message_Num + 1) {
                lock_guard<mutex> lock(mtx);
            ...
            // 流量控制 - 当发送窗口接近拥塞窗口时减缓发送速度
            if (Next_Seq - Base_Seq > cwnd * 0.8) {
...
int main() {
   ...
   // cin >> CLIENT_PORT;
    ...
    do {
        cout << "---------------------------------------------------" << endl;
        cout << "Please choose an option：\n1. SEND FILE   2. EXIT  3. 设置超时模拟  4. 设置超时时间\n"<< "Select：";
        cin >> choice;
        ...
        if (choice == 1) {
            ...
            bool enable;
            cout << "是否启用超时模拟? (1-启用/0-禁用): ";
            cin >> enable;
            sender.Set_SimulateTimeout(enable);
        }
        else if (choice == 4) {
            int timeout_ms;
            cout << "请输入新的超时时间(ms): ";
            cin >> timeout_ms;
            sender.Set_Timeout(timeout_ms);
        }
        else {
